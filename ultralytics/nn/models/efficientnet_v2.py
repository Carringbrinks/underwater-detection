...
class EfficientNet(nn.Module):
    ...
    def __init__(
        ...
        self.channel = [40, 56, 136, 232]

        if model_type == "backbone_rfa":
            self.rfa_conv_0 = RFAConv(16, 16, 3)
            self.rfa_conv_1 = RFAConv(40, 40, 3)
            self.rfa_conv_2 = RFAConv(56, 56, 3)
            self.rfa_conv_3 = RFAConv(112, 112, 3)
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # x = self.blocks(x)
            if self.model_type == "backbone_rfa":
...
@register_model
def tf_efficientnetv2_b3_rfa(pretrained=False, **kwargs) -> EfficientNet:
    """ EfficientNet-V2-B3. Tensorflow compatible variant """
    kwargs.setdefault('bn_eps', BN_EPS_TF_DEFAULT)
    kwargs.setdefault('pad_type', 'same')
    model = _gen_efficientnetv2_base(
